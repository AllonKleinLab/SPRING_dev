...
define(["require", "exports", "d3", "./main"], function (require, exports, d3, main_1) {
    ...
    class PAGA {
        ...
        loadData() {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    const data = yield d3.json(main_1.project_directory + '/PAGA_data.json' + '?_=' + this.noCache);
                    if (data !== undefined) {
                        this.PAGA_data = data;
        ...
        adjust_min_edge_weight() {
            let min_weight = Math.exp(parseFloat(d3.select('#PAGA_min_edge_weight_slider').node().value) / 20) / Math.exp(100 / 20);
        ...
        PAGA_redraw() {
            let node_scale = Math.pow((d3.select('#PAGA_node_size_slider').node().value / 40), 1.8);
            let edge_scale = Math.pow((d3.select('#PAGA_edge_width_slider').node().value / 40), 3);
            d3.selectAll('.PAGA_node')
                .attr('cx', d => {

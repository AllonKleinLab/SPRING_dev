...
define(["require", "exports", "d3", "./colorpicker_layout", "./main", "./util"], function (require, exports, d3, colorpicker_layout_1, main_1, util_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class ColorBar {
        constructor(color_menu_genes) {
            this.downloadRankedTerms = () => {
                let num_selected = 0;
                let termcol = [];
                let scorecol = [];
            ...
            this.get_hover_cells = e => {
                ...
                for (let i = 0; i < main_1.forceLayout.all_nodes.length; i++) {
                    if (main_1.forceLayout.all_outlines[i].selected) {
                        ...
                        if (rad < main_1.forceLayout.all_nodes[i].scale.x * 20) {
                            hover_cells.push(i);
                        }
                    }
                }
        ...
        static create(color_menu_genes) {
        ...
        update_tints() {
            for (let i = 0; i < main_1.forceLayout.base_colors.length; i++) {
                const rgb = main_1.forceLayout.base_colors[i];
                main_1.forceLayout.all_nodes[i].tint = util_1.rgbToHex(rgb.r, rgb.g, rgb.b);
            }
        }
        setNodeColors() {
            if (document.getElementById('gradient_button').checked) {
                ...
                for (let i = 0; i < main_1.forceLayout.base_colors.length; i++) {
                    main_1.forceLayout.base_colors[i] = d3.rgb(this.gradient_color(color_array[i]));
                }
                this.update_tints();
            }
                ...
                $.ajax({
                    ...
                    success: data => {
                        ...
                        for (let i = 0; i < main_1.forceLayout.all_nodes.length; i++) {
                            ...
                            main_1.forceLayout.base_colors[i] = { r: 0, g: Math.floor(gg * 255), b: 0 };
                        }
        ...
        updateColorMax() {
            ...
            if (document.getElementById('labels_button').checked) {
                for (let i = 0; i < main_1.forceLayout.base_colors.length; i++) {
                    const rr = Math.floor(this.normalize_one_val(main_1.forceLayout.base_colors[i].r) * 255);
                    const gg = Math.floor(this.normalize_one_val(main_1.forceLayout.base_colors[i].g) * 255);
                    const bb = Math.floor(this.normalize_one_val(main_1.forceLayout.base_colors[i].b) * 255);
                    main_1.forceLayout.all_nodes[i].tint = util_1.rgbToHex(rr, gg, bb);
                }
                // update_tints();
            }
        }
        ...
        slider_select_update() {
            ...
            if (document.getElementById('labels_button').checked) {
                color_array = main_1.forceLayout.base_colors.map(this.average_color);
            }
            if (color_array != null) {
                for (let i = 0; i < main_1.forceLayout.all_nodes.length; i++) {
        ...
        update_slider() {
            ...
            if (document.getElementById('labels_button').checked) {
                ...
                let max = parseInt(d3.max(main_1.forceLayout.base_colors.map(this.max_color)), 10);
                if (max === 0) {
                    max = 255;
                }
                this.color_max = max;
        ...
        getRankedText(tracks, version) {
            ...
            for (const i in main_1.forceLayout.all_outlines) {
                if (main_1.forceLayout.all_outlines[i].selected) {
        ...
        make_legend(cat_color_map, cat_label_list) {
            ...
            d3.select('#label_column')
                ...
                d3.select(nodes[i])
                    ...
                    .on('click', () => {
                    colorpicker_layout_1.show_colorpicker_popup(d);
                });
                d3.select(nodes[i])
                    .append('div')
        ...
        shrinkNodes(scale, numsteps, my_nodes, all_nodes) {
            ...
            const refreshIntervalId = setInterval(() => {
                if (scale < 1) {
                    ...
                    for (let ii in nodes) {
                        const i = nodes[ii];
                        main_1.forceLayout.all_outlines[i].scale.set(scale * current_radii[ii]);

...
define(["require", "exports", "d3", "./LineSprite", "./main", "./util", "./settings_script", "./rotation_script", "./file_helper"], function (require, exports, d3, LineSprite_1, main_1, util_1, settings_script_1, rotation_script_1, file_helper_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class ForceLayout {
        constructor() {
            this.load_edges = (all_nodes, sprites) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    const edgesText = yield d3.text(main_1.project_directory + '/edges.csv');
                    edgesText.split('\n').forEach((entry, index) => {
                        if (entry.length > 0) {
                            ...
                            let color = 6579301;
                            let s = new LineSprite_1.LineSprite(4, color, x1, y1, x2, y2);
                            s._color = color;
            ...
            this.animation = () => {
                ...
                const filePath = main_1.project_directory + '/animation.txt';
                d3.text(filePath)
                    .then(data => {
            ...
            this.downloadSelection = () => {
                ...
                d3.text(cell_filter_filename).then(cellText => {
                    ...
                    util_1.downloadFile(text, 'selected_cells.txt');
                });
            };
            this.downloadCoordinates = () => {
                ...
                util_1.downloadFile(text, 'coordinates.txt');
            };
            this.initiateButtons = () => {
                ...
                d3.select('#save_coords')
                    ...
                    if (this.mutable) {
                        ...
                        let name = window.location.search;
                        let path = 'coordinates/' + name.slice(9, name.length).split('/')[1] + '_coordinates.' + main_1.sub_directory + '.txt';
                        $.ajax({
                ...
                d3.select('#rotation_update').on('click', () => rotation_script_1.rotation_update());
                ...
                d3.select('#show_download__selected_expr_popup').on('click', () => main_1.downloadSelectedExpr.show_downloadSelectedExpr_popup());
                d3.select('#show_make_new_SPRINGplot_popup').on('click', () => main_1.springPlot.show_make_new_SPRINGplot_popup());
                ...
                d3.select('#show_doublet_popup').on('click', () => main_1.springPlot.show_make_new_SPRINGplot_popup());
            ...
            this.download_sprite_as_png = (renderer, sprite, fileName) => {
        ...
        static create() {
            return __awaiter(this, void 0, void 0, function* () {
                if (!this._instance) {
                    ...
                    throw new Error('ForceLayout.create() has already been called, get the existing instance with ForceLayout.instance!');
                }
            });
        }
        ...
        loadData() {
            return __awaiter(this, void 0, void 0, function* () {
                const filePath = main_1.project_directory + '/mutability.txt';
                try {
                ...
                const text = yield file_helper_1.getData('coordinates', 'txt');
                if (!text.push) {
                ...
                yield this.load_edges(this.all_nodes, this.sprites);
            });
        }
        create_sprites(totalSprites) {
            for (let i = 0; i < totalSprites; i++) {
                ...
                dude.anchor.set(0.5);
                dude.scale.set((0.5 * 32) / util_1.SPRITE_IMG_WIDTH);
                dude.x = this.coordinates[i][0];
                dude.y = this.coordinates[i][1];
                dude.tint = parseInt(util_1.rgbToHex(0, 0, 0), 16);
                dude.alpha = 1;
        ...
        dragstarted() {
            if (main_1.selectionScript.selection_mode === 'drag_pan_zoom') {
                ...
                for (let i = 0; i < this.all_nodes.length; i++) {
                    if (this.all_outlines[i].selected) {
                        let rad = Math.sqrt(Math.pow((this.all_nodes[i].x - x), 2) + Math.pow((this.all_nodes[i].y - y), 2));
                        if (rad < this.all_nodes[i].scale.x * 20) {
                            clicked_pos_sel = true;
                        }
                    }
                    if (this.all_outlines[i].compared) {
                        const newRad = Math.sqrt(Math.pow((this.all_nodes[i].x - x), 2) + Math.pow((this.all_nodes[i].y - y), 2));
        ...
        dragged() {
            ...
            for (let i = 0; i < this.all_edges.length; i++) {
                if (this.all_nodes[this.all_edge_ends[i].source].beingDragged ||
        ...
        redraw() {
            if (!this.being_dragged && d3.event.sourceEvent) {
                ...
                main_1.cloneViewer.clone_edge_container.position = this.sprites.position;
                main_1.cloneViewer.clone_edge_container.scale = this.sprites.scale;
                main_1.cloneViewer.clone_sprites.position = this.sprites.position;
                main_1.cloneViewer.clone_sprites.scale = this.sprites.scale;
                // text_container.position = sprites.position;
                // text_container.scale = sprites.scale;
                d3.select('#vis').attr('transform', 'translate(' +

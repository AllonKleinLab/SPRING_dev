function forceLayout(project_directory, sub_directory, callback) {
	...
	if (d3.select("#sound_toggle").select("img").attr("src") == "scripts_dynamic/sound_effects/icon_speaker.svg") {
		var snd = new Audio("scripts_dynamic/sound_effects/opennew_sound.wav"); snd.play(); }
	...
	d3.text(coordinates_filename, function(text) {
		...
		loadColors();	
		load_edges();
		force_simulation(graphData_filename);
	...
	function force_simulation(graph_file, finalMinAlpha=0.01, theta=.9, linkStrength=2, resets=0) {
		d3.json(graph_file, function(error, graph) {
			var nticks = 0;
		    var alphaResetCount = 0;
		    const maxResets = 0;
			...
			.on('tick', function() {
				nticks += 1;
				if (edge_container.visible) {
					toggle_edges();
				}
				for (var i = 0; i < simulation.nodes().length; i++) {
					if (all_outlines[i].selected | nticks == 1){
						simulation.nodes()[i].x = all_nodes[i].x;
						simulation.nodes()[i].y = all_nodes[i].y;
					} else {
						all_nodes[i].x = simulation.nodes()[i].x;
						all_nodes[i].y = simulation.nodes()[i].y; 
						all_outlines[i].x = simulation.nodes()[i].x;
						all_outlines[i].y = simulation.nodes()[i].y;	
					}
			    }
			})
			.stop();
...
function downloadFile(text,name) {
	if (d3.select("#sound_toggle").select("img").attr("src") == "scripts_dynamic/sound_effects/icon_speaker.svg") {
		var snd = new Audio("scripts_dynamic/sound_effects/download_sound.wav"); snd.play(); }

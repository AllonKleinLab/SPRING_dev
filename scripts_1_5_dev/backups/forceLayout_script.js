function forceLayout(project_directory, callback) {
	...
	if (d3.select("#sound_toggle").select("img").attr("src") == "scripts_1_5/sound_effects/icon_speaker.svg") {
		var snd = new Audio("scripts_1_5/sound_effects/opennew_sound.wav"); snd.play(); }
	...
	d3.text(project_directory + '/mutability.txt', function(text) { 
		mutable = text; 
	...
	d3.json(graphData_filename, function(error, graph) {
		...
		if (num_fixed == graph.nodes.length) {
...
function loadColors() {
    ...
    sub_dir=graph_directory + "/sub";
    console.log(base_dir);
    console.log(sub_dir);
    $.ajax({
        url:"cgi-bin/load_counts_npz.py",
        type:"POST",
...
function initiateButtons() {
	...
	d3.select('#save_coords').select('button').on("click",function() {
		if (mutable.slice(0,5) != 'false') {
			...
			path = "coordinates/"+name.slice(9,name.length).split('/')[1] + "_coordinates.txt";
			$.ajax({

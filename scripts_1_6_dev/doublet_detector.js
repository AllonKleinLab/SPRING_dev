function doublet_setup() {
  ...
  var text_box = popup
    ...
    .append('text')
    .html(
      'Predict mixed-celltype doublets. Uses a kNN classifier to identify transcriptomes that resemble simulated doublets. <br><br> <b><i>k </i> : </b> the number neighbors used in the classifier <br> <b><i>r </i> : </b> the ratio of simulated doublets to observed cells <br> <b><i>f </i> : </b> the expected doublet rate',
    );

  var doublet_notify_popup = d3
    .select('#force_layout')
    .append('div')
    ...
    if (mutable) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          d3.json(graph_directory + '/' + sub_directory + '/color_stats.json' + '?_=' + noCache, function(data) {
            color_stats = data;
          });
          d3.text(graph_directory + '/' + sub_directory + '/color_data_gene_sets.csv' + '?_=' + noCache, function(
            text,
          ) {
...
function show_doublet_notification() {
  var mywidth = parseInt(
    d3
      .select('#doublet_notification')
      .style('width')
      .split('px')[0],
  );
  var svg_width = parseInt(
    d3
      .select('svg')
      .style('width')
      .split('px')[0],

...
function PAGA_redraw() {
  ...
  d3.selectAll('.PAGA_link')
    ...
    .attr('x1', function(d) {
      return PAGA_node_dict[d.source].coordinates[0];
    })
    .attr('y1', function(d) {
      return PAGA_node_dict[d.source].coordinates[1];
    })
    .attr('x2', function(d) {
      return PAGA_node_dict[d.target].coordinates[0];
    })
    .attr('y2', function(d) {
      return PAGA_node_dict[d.target].coordinates[1];
    })
    .attr('stroke-width', function(d) {
      return d.weight * edge_scale;
    })
    .style('visibility', function(d) {
      if (d.weight > PAGA_data.edge_weight_meta.min_edge_weight) {
        return 'visible';
      } else {
        return 'hidden';
      }

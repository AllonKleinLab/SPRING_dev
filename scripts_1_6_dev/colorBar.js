function colorBar(project_directory, color_menu_genes) {
  ...
  dispatch.on('load', function(data, tag) {
    if (tag == 'gene_sets') {
      var select = gradientMenu;
    } else if (tag == 'all_genes') {
      var select = greenMenu;
    } else {
      var select = labelsMenu;
    }
  ...
  var right_bracket_label = slider
    .append('text', '#track-overlay')
    .attr('class', 'bracket_label')
  ...
  d3.json(project_directory + '/color_stats.json' + '?_=' + noCache, function(data) {
    color_stats = data;
  });
  ...
  function showRankedGenes() {
    if (color_stats != null) {
      if (
        ...
        var snd = new Audio('scripts_1_6_dev/sound_effects/openclose_sound.wav');
        snd.play();
      }
      //setNodeColors();
      hideRankedTerms();
  ...
  function downloadFile(text, name) {
    if (
      ...
      var snd = new Audio('scripts_1_6_dev/sound_effects/download_sound.wav');
      snd.play();
    }
    ...
    if (num_selected == 0 && rankedMask.attr('x') == -200) {
      ...
      if (rankedMask.attr('x') == -200) {
        if (document.getElementById('gradient_button').checked) {
          ...
          var tracks = all_gene_color_array;
          var sparse_version = 1;
        }
...
function count_clusters() {
  ...
  if (name.length > 0) {
    ...
    counts = {};
    Object.keys(cat_color_map).forEach(function(d) {
      counts[d] = 0;
    });
    for (i = 0; i < all_nodes.length; i++) {
      if (all_outlines[i].selected || all_outlines[i].compared) {

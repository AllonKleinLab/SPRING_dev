export const cluster_setup = project_directory => {
  ...
  d3.select('#cluster_view_button').on('click', function() {
    ...
    let Nmax = Object.keys(clustering_data.clusters).length;
    if (N > Nmax || N < 1) {
      sweetAlert({
  ...
  const cluster_box_dragstarted = () => {
    d3.event.sourceEvent.stopPropagation();
  };
  const cluster_box_dragged = () => {
    let cx = parseFloat(
      d3
  ...
  const update_cluster_labels_box_dragged = () => {
    let cx = parseFloat(
      d3
  ...
  const update_cluster_labels_box_dragended = () => {
    return;
  };
};

export const hide_create_cluster_box = () => {
  d3.select('#create_cluster_box').style('visibility', 'hidden');
};

export const show_create_cluster_box = () => {
...
export const showClusterDropdown = () => {
...
export const view_current_clusters = () => {
  categorical_coloring_data['Current clustering'] = clustering_data.clusters[current_clus_name];
  ...
  let cat_color_map = clustering_data.clusters[current_clus_name].label_colors;
  let cat_label_list = clustering_data.clusters[current_clus_name].label_list;
...
export const color_nodes = (cat_color_map, cat_label_list) => {
  d3.select('.node')
    .selectAll('circle')
...
export const make_new_clustering = () => {
  show_create_cluster_box();
};

export const toggle_spectrum = () => {
...
export const show_spectrum = () => {
  ...
  let maxval = d3.max(gap_data, function(d) {
    return d.y;
  });
  x.domain(
  ...
  let argmax = clustering_data.spectral_info.argmax;
  svg
    .append('line')
  ...
  let clusnum = clustering_data.spectral_info.argmax.toString();
  svg
    .append('text')
...
export const show_update_cluster_labels_box = () => {
...
export const hide_update_cluster_labels_box = () => {
  d3.select('#update_cluster_labels_box').style('visibility', 'hidden');
};

export const apply_legend = () => {
  new_clus = Object();
  new_colors = Object();
  new_labels = [];
  new_names = [];
...
export const save_cluster_data = () => {
...
export const download_legend_image = () => {
...
export const download_clustering = () => {
  let text = '';
  let label_list = clustering_data.clusters[current_clus_name].label_list;
  d3.select('.node')
    .selectAll('circle')
    .sort(function(a, b) {

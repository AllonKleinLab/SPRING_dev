...
if len(found_bad) > 0:
    all_errors.append(
        'Enter a <font color="red">cell subset name</font> without the following characters: <font face="courier">%s</font><br>' % ('   '.join(found_bad)))
...
if not do_the_rest:
    ...
    try:
        ...
        out_dir = 'downloads/'
        params_filename = "downloads/" + selection_name + \
            "." + rand_suffix + "/download_params.pickle"
        ...
        subprocess.call(
            ["cgi-bin/download_expression.submit.sh", params_filename])
    except:

...
class ForceAtlas2:
    ...
    def forceatlas2(self,
        ...
        for _i in niters:
            ...
            if writefile != None and _i%3==0:
                write_pos = numpy.zeros((len(nodes),2))
                for i,n in enumerate(nodes):
                    write_pos[i,0] = n.x
                    write_pos[i,1] = n.y
                write_pos = write_pos / 5.0
                write_pos = write_pos - numpy.min(write_pos, axis = 0) - numpy.ptp(write_pos, axis = 0) / 2.0
                write_pos[:,0] = write_pos[:,0]  + 750
                write_pos[:,1] = write_pos[:,1]  + 250
                outline = ','.join([repr(x) for x in write_pos[:,0]])+';'+','.join([repr(x) for x in write_pos[:,1]])
    ...
    def forceatlas2_networkx_layout(self, G, pos=None, iterations=100, writefile=None):
        import networkx
        ...
        if pos is None:
            l = self.forceatlas2(M, pos=None, iterations=iterations, writefile=writefile)
        else:
            poslist = numpy.asarray([pos[i] for i in G.nodes()])
            l = self.forceatlas2(M, pos=poslist, iterations=iterations, writefile=writefile)

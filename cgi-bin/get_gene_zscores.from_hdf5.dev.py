...
update_log(logf, '%i selected cells; %i compared cells' %
           (len(sel_filter), len(comp_filter)), False)
...
if len(sel_filter) > 0:
    ...
    cell_filter = np.array(
        np.load(sub_dir + '/cell_filter.npy')[sel_filter], dtype=str)
...
if len(comp_filter) > 0:
    t0 = time.time()
    cell_filter = np.array(
        np.load(sub_dir + '/cell_filter.npy')[comp_filter], dtype=str)
    totals = np.zeros(len(gene_list), dtype=float)

...
function cluster_setup(project_directory) {
	...
	d3.json(project_directory+"/clustering_data.json", function(data) {
	...
	current_clus_name = 'Cluster1';
	last_clus_name = 'Cluster1';
...
function show_spectrum() {
	...
	svg.append('line')
		.attr({x1:x(3), y1:y(0), x2:x(3), y2:y(maxval), stroke:'rgba(100,100,100,1)'})
    ...
    var clusnum = (clustering_data["spectral_info"]["argmax"]+1).toString();
    svg.append("text")
   	  .attr("transform",
    ...
    .style("color","white")
    .text('The recommended number of clusters is '+clusnum+', because this is the number'
         + ' with the highest spectral gap. Spectral gap measures the difference in '
...
function show_update_cluster_labels_box() {
	...
	if (d3.select("#update_cluster_labels_box").style("visibility") == "hidden") {
		...
		d3.select("#save_legend_button").on("click",save_legend);
...
function save_legend() {
	...
	if (new_name_set.size < new_names.length) {
		...
		if (document.getElementById('labels_button').checked) {
			if (document.getElementById('labels_menu').value == current_clus_name) {
				d3.select("#label_column").selectAll("div").remove();
				d3.select("#count_column").selectAll("div").remove();

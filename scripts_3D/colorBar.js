function colorBar(project_directory, color_menu_genes) {
	...
	*/
	...
	function update_tints() {
		for (var i = 0; i < base_colors.length; i++) {
			...
			node_dict[i].setColor(rgbToHex(rgb.r,rgb.g,rgb.b));
		}
	...
	*/
	...
	d3.select('#color_chooser').append('svg')
		.style('width','300px')
		.style('height','100px')
	...
	var slider_scale = d3.scale.linear()
		.domain([0, 10])
		.range([0, svg_width * 7 / 9])
		.clamp(true);

	var slider = d3.select("svg").append("g")
		.attr("class","colorbar_item")
		.attr("id", "slider")
		.attr("transform", "translate(" + svg_width/9 + "," + 42 + ")");
...
// 		.attr("height",d3.select("svg").attr("height"));
	...
	slider.call(d3.behavior.drag()
		.on("dragstart", function() {
			var cx = d3.event.sourceEvent.x-svg_width/9;
